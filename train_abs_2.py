...
def parse_args():
    ...
    parser.add_argument('-n', "--abs_nums", action="append", type=int, default=[])
...
def main():
    ...
    if "compile" in args.dataset_path:
        abs_nums = [abs_num if abs_num >= 0 else None for abs_num in args.abs_nums]
        abs_config = {"abs_type": args.abs_type, "num_iters": args.num_iters, "top": abs_nums}
        train_loader, test_loader = utils.compile_loader(args.batch_size, abs_config=abs_config)
    ...
    model = HierarchicalStateSpaceModel(
        ...
        latent_n=len(train_loader.dataset.abs2idx),
    ...
    while b_idx <= args.max_iters:
        ...
        for train_obs_list, train_action_list, train_next_obs_list, train_z_list, train_m_list in tqdm.tqdm(train_loader):
            ...
            loss = model.forward_z_batch(train_obs_list, train_action_list, train_next_obs_list, train_z_list, train_m_list, seq_size, init_size)
            loss.backward()
