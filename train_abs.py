...
from hssm_rl import HierarchicalStateSpaceModel
import utils
import modules
from modules import (
    GridDecoder,
...
def parse_args():
    ...
    # abstractions
    parser.add_argument("--abs_type", required=True)
...
def main():
    ...
    if "compile" in args.dataset_path:
        train_loader, test_loader = utils.compile_loader(args.batch_size, args.abs_type)
    ...
    # test data
    pre_test_full_state_list, pre_test_full_action_list, pre_test_full_next_state_list, pre_test_full_z_list, pre_test_full_m_list = iter(test_loader).next()
    pre_test_full_state_list = pre_test_full_state_list.to(device)
    pre_test_full_action_list = pre_test_full_action_list.to(device)
    pre_test_full_next_state_list = pre_test_full_next_state_list.to(device)
    pre_test_full_z_list = pre_test_full_z_list.to(device)
    pre_test_full_m_list = pre_test_full_m_list.to(device)
    ...
    while b_idx <= args.max_iters:
        # for each batch
        for train_obs_list, train_action_list, train_next_obs_list, train_z_list, train_m_list in tqdm.tqdm(train_loader):
            b_idx += 1
            # mask temp annealing
            if args.beta_anneal:
                model.mask_beta = (
            ...
            train_next_obs_list = train_next_obs_list.to(device)
            train_z_list = train_z_list.to(device)
            train_m_list = train_m_list.to(device)
            ...
            for i, (obss, actions, next_obss, zs, ms) in \
                    enumerate(zip(train_obs_list, train_action_list, train_next_obs_list, train_z_list, train_m_list)):
                for j in range(init_size, init_size + seq_size):
                    obs = obss[j]
                    action = actions[j]
                    next_obs = next_obss[j]
                    z = zs[j]
                    m = ms[j]
                    if z == -1:
                        continue
                    if m == 1:
                        boundary_state = model.initial_boundary_state(obs.float())
                        hidden_state = None
                    action_logits, hidden_state = model.forward_z(
                            z, obs.float(), hidden_state)
                    terminate_logits, boundary_state = model.forward_z_terminates(
                            next_obs.float(), action, boundary_state)
                    loss += F.cross_entropy(action_logits.unsqueeze(0), action.unsqueeze(0)) + \
                            F.cross_entropy(terminate_logits.unsqueeze(0), (1 - ms[j+1]).unsqueeze(0))
            loss.backward()
            ...
            if b_idx % 2000 == 0:
                ...
                os.makedirs(exp_dir, exist_ok=True)
                torch.save(
                    model, os.path.join(exp_dir, f"model-{b_idx}.ckpt")
                )
